...
impl<'a> I2cSensorDriver<'a> {
    ...
    /// Pitch = angle_buf[1];
    /// Yaw = angle_buf[2];
    pub fn read_angle(&mut self, angle_buf: &mut [f32; 3]) -> Result<(), I2cSensorError> {
        ...
        angle_buf[0] = i16::from_le_bytes([angle_raw[0], angle_raw[1]]) as f32 / 32768.0 * 180.0;
        angle_buf[1] = i16::from_le_bytes([angle_raw[2], angle_raw[3]]) as f32 / 32768.0 * 180.0;
        angle_buf[2] = i16::from_le_bytes([angle_raw[4], angle_raw[5]]) as f32 / 32768.0 * 180.0;
...
impl<'a> UartSensorDriver<'a> {
    ...
    pub fn read_distance(&mut self, dis_buf: &mut [u16; 1]) -> Result<(), UartSensorError> {
        ...
        dis_buf[0] = u16::from_le_bytes([distance_raw[4], distance_raw[3]]);
